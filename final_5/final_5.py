...
class DataLoader():
    ...
    def load_batch(self, batch_size=1):
        ...
        for i in range(self.n_batches - 1):
            ...
            for img_path in path_u:
                img = cv2.imread('./train3/' + img_path)
    ...
    def loadAllCSV(self):
        with open("./train3.csv", "r") as csvfile:
            # 讀取 CSV 檔案內容
        ...
        with open("./val3.csv", "r") as csvfile:
            # 讀取 CSV 檔案內容
            train_data_dict = csv.reader(csvfile, delimiter=',')
            arr = []
            for index, row in enumerate(train_data_dict):
            ...
            if (epoch < skip):
                ...
                for batch_i, (imgs, coords) in enumerate(self.data_loader.load_batch(batch_size)):
                    ...
                    if (batch_i + 1) % sample_interval == 0:
                        self.validation(epoch, batch_i + 1)
                self.validation(epoch, batch_size)
                self.CNN_Network.save_weights('./saved_model2/CNN_Network_on_epoch_%d.h5' % (epoch))

    def validation(self, epoch, num_batch):
        Xte, Xte_labels = self.data_loader.load_data(batch_size=10)
...
if __name__ == '__main__':
    ...
    #     my_CNN_Model.train(epochs=200, batch_size=16, sample_interval=50, skip=0)

    #     testing model
    start_time = datetime.datetime.now()
    my_CNN = ConvolutionalNeuralNetworks()
    elapsed_time = datetime.datetime.now() - start_time
    print("initConvolutionalNeuralNetworks", elapsed_time)

    my_CNN.CNN_Network.load_weights('./saved_model3/CNN_Network_on_epoch_105.h5')
    ...
    for pred_label, label in zip(pred_labels, labels):
        if (pred_label.argmax() != label.argmax()):
            print("predict: ", pred_label.argmax(), " -- ", "true: ", label.argmax())
    print("Validation acc: " + str(
